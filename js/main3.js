...
function createMap(){
    ...
    //var genLayer = new L.geoJSON().addTo(map);
    //var recLayer = new L.geoJSON().addTo(map);
...
function createPopup(properties, attribute, layer, radius, setName){
    ...
    var year = attribute.split("_")[1];
     ...
     if (setName == "genMSW"){
        ...
        popupContent += "<p><b>Material Recovered - Recycling/Composting (" + year + "):</b></p>" + "<p>" + properties[attribute] + " %</p>";
    }
...
function createPropSymbols(data, map, attributes, setName){
    var genLayer = L.layerGroup();
    var recLayer = L.layerGroup();
    
    //create a Leaflet GeoJSON layer and add it to the map
    L.geoJson(data, {
        pointToLayer: function(feature, latlng){
            return pointToLayer(feature, latlng, attributes, setName);
        },
       onEachFeature: function(feature, layer) {
            if (feature.properties.Variable == "Municipal Waste Generated Per Capita (kg)"){
                layer.addTo(genLayer.addTo(map));
            } else if (feature.properties.Variable == "% Material Recovery (Recycling + Composting)"){
                layer.addTo(recLayer.addTo(map));
            }
        }
    }).addTo(map);
  
    createLayerControl(genLayer, recLayer); 
}; 

function createLayerControl(genLayer, recLayer){
    var baseLayers = null;
    var overlays = {
...
function setUpFunction(map, genData, recData, genAttributes, recAttributes){
    ...
    createPropSymbols(genData, map, genAttributes, genSetName);
